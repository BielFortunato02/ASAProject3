...
def solve_toy_distribution(input_data):
    ...
    for country_id, max_export, min_delivery in country_constraints:
        ...
        # Apply export and distribution constraints
        if max_export > 0:  # Apply only if there's a positive max export limit
            problem += country_assignments <= max_export, f"Country_{country_id}_Max_Export"
        if min_delivery > 0:  # Apply only if there's a minimum delivery requirement
